<template>
  ...
  <div class="table">
    <el-table :data="list" style="width: 100%" height="500px">
      <el-table-column v-for="item in tableLabel" :key="item.prop" :label="item.label" :prop="item.prop" :width="item.width ? item.width : 205" />
    ...
    <el-pagination small background layout="prev, pager, next" :total="config.total" @current-change="changePage" class="pager mt-4" />
  </div>
...
export default defineComponent({
  setup() {
    ...
    const getUserData = async (config) => {
      let res = await request.getUserData(config);
      ...
      proxy.$refs.userForm.validate(async (valid) => {
        if (valid) {
          if (action.value == "add") {
            ...
            let res = await request.addUser(formUser);
            if (res) {
            ...
            let res = await request.editUser(formUser);
            if (res) {
              // console.log(proxy.$refs);
              dialogVisible.value = false;
              proxy.$refs.userForm.resetFields();
